...
require_once(PATH_CLASSES."/Produto.php");
	...
	$catalogoPath = PATH_CATALOGO . "/smt/" ;
	$catalogoURL =  PATH_CATALOGO_URL . "/smt/" ;
	$extensaoProduto = "produto";
	
	
	if (isset($_REQUEST['id'])) {
		$id = $_REQUEST['id'];
		$file = PATH_CATALOGO . "/smt/$id".".". $extensaoProduto;
		if (!file_exists($file)) {
			$id = -1;
		}  
...
<!-- SMT -->	
<?
	if ( (isset($action) && ($action == "")) || (isset($action) && ($action === Link::getAlias(31)))  )  {
				...
				if (!in_array($value,array(".",".."))) 
				{
					...
					if ($ext == $extensaoProduto) {
						...
						?>
							...
							<div class="box boxEffect2" onclick="go('<?=Link::getLink(31)."&id=".$base ?>')" >
									<div class="inner">
...
<?
		...
		if (file_exists($file)) {
			$produto = new Produto();
			require_once($file);
			$produto->exibirProduto($id,$action,$catalogoURL, $catalogoPath,31);	
