...
*/	
	...
	$catalogoPath = PATH_CATALOGO . "/nozzlesfeeders/" ;
	$catalogoURL =  PATH_CATALOGO_URL . "/nozzlesfeeders/" ;
	...
	if (isset($_REQUEST['id'])) {
		$id = $_REQUEST['id'];
		$file = PATH_CATALOGO . "/nozzlesfeeders/$id".".". $extensaoProduto;
		if (!file_exists($file)) {
			$id = -1;
		}  
...
<!-- SMT -->	
<?
	if ( (isset($action) && ($action == "")) || (isset($action) && ($action === Link::getAlias(33)))  )  {
				...
				if (!in_array($value,array(".",".."))) 
				{
					...
					if ($ext == $extensaoProduto) {
						...
						?>
							...
							<div class="box boxEffect2" onclick="go('<?=Link::getLink(33)."&id=".$base ?>')" >
									<div class="inner">
...
<?
		...
		if (file_exists($file)) {
			$produto = new Produto();
			require_once($file);
			$produto->exibirProduto($id,$action,$catalogoURL, $catalogoPath,33);	
